...
describe('Filters:', function () {
  ...
  describe('filtersController functions', function () {
    ...
    it('should add range filter with unit', function () {
      cfg.settings.data.filters = [{field: 'rangeField', style: 'RANGE', minCount: 0, unit:'fileSize'}];

      controllerService('FiltersCtrl', {$scope: scope, filterService: _filterService});
      var facet = {isSelected: true, model:[15000,200000], name:'name', display:'display', style:'RANGE', filter:'rangeField', field:'rangeField'};
      scope.addRangeFilter(facet);
      httpMock.expectJSONP(new RegExp(escapeRegExp('stats=true'))).respond({stats:{stats_fields:{rangeField:{min:10000, max:5000000}}}});  // stats query for range values
      ...
      cfg.settings.data.filters[0].values[0].selectedUnit = 'KB';
      cfg.settings.data.filters[0].values[0].convert(cfg.settings.data.filters[0].values[0]);

      expect(cfg.settings.data.filters[0].values[0].storedUnit).toEqual('KB');
      expect(cfg.settings.data.filters[0].values[0].min).toEqual(10);
      expect(cfg.settings.data.filters[0].values[0].max).toEqual(5000);
